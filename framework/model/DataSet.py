...
class TGCDataSet(Dataset):
    def __init__(self, file_path, neg_size, hist_len, feature_path, directed, transform=None):
        ...
        # Dynamic neg_table: size proportional to node_dim instead of fixed 1e8
        self.neg_table_size = max(self.node_dim * 10000, 100000)
    ...
    def get_feature(self):
        # Try .npy first (fast binary), fall back to .emb text format
        npy_path = self.feature_path.replace('.emb', '.npy')
        meta_path = npy_path.replace('.npy', '_ids.npy')
        if os.path.exists(npy_path) and os.path.exists(meta_path):
            features = np.load(npy_path)
            ids = np.load(meta_path)
            node_emb = dict(zip(ids, features))
        else:
            node_emb = dict()
    ...
    def negative_sampling(self):
        ...
        sampled_nodes = self.neg_table[rand_idx].astype(np.int64)
